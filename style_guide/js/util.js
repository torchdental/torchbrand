...
E.g.
  ...
  listItemBuilder(); // returns "listItem pet"
  listItemBuilder(null, 'selected'); // returns "listItem listItem--selected pet pet--selected"
  listItemBuilder('name', ['a', 'b']); // returns "listItem-name listItem-name--a listItem-name--b pet-name pet-name--a pet-name--b"
  listItemBuilder('header', {inverted: true, striped: true}) // returns "listItem-header listItem0header--inverted listItem-header--striped pet-header pet-header--inverted pet-header--striped"

You can also override the separators, e.g.
const otherBuilder = makeBem('list-item', {e: '__' });
otherBuilder('header', 'striped') // returns 'list-item__header list-item__header--striped'
 */
export const makeBem = (...args) => {
  let e = '-';
  let m = '--';
  const blocks = [];
  args.forEach((arg) => {
